...
public class CarArrayPreService {
    ...
    //todo : 2. cars 배열을 브랜드 이름ㅇ르 기준으로 오름차순 정렬
        ...
        //SELECET * FROM CAR ORDER BY BRAND = ASC;
        ...
        cars[j].brand   cars[j+1].brand     브램드 이름을 비교했을 때
        ...
        for(int i=0;i<cars.length;i++){
            for(int j=0;j<cars.length;j++){
                if(cars[j].brand.compareTo(cars[j+1].brand)>0){
                    ...
                    cars[j] = cars[j+1]; //비교했을 때 가나다 순으로 앞에 있어야 하느느 단어위치를 앞으로 변경
                    cars[j+1] = temp; //뒤에 있어야 하는데 앞에 이는 단어를 뒤로 변경
                    /*
                    바지/가방
    ...
    //todo : 4. 다른 배열 값과 같은지 비교 비교 결과 같은지 확인
        ...
        System.out.println("cars와 carsEqual 안에 들어있는 차들은 같은가? : " +areEquals);
