...
public class ListEx {
    ...
    * 리스트는 인터페이스로 정의
    * 초기에는 Vector를 사용했지만 메모리사용과 시간소모가 많아 아래 클래스를 출시함
    * 대표적인 클래스 : ArrayList(조회 빠름) , LinkedList(삽인, 삭제가 빠름)
    *
    * 특징
    * 순서 존재 : 저장된 요소(값)들은 인덱스를 통해 0번부터 저장되고, 순차적으로 접근 가능
    * 중복 허용 : 동일한 요소(값)을 여러번 저장할 수 있음
    * 동적 크기 : 배열과 달리 리스트는 크기가 유동적으로 변경될 수 있음
    * null 허용: null 값을 요소(값)으로 저장할 수 있음.
    *
    * 주요 메서드(기능)
    * add (E e)          : 리스트 끝에 요소(element = 값)을 추가
    * get(int index)     : 주어진 index의 요소 값을 반환
    ...
    * */
    ...
    public static void main(String[] args) {
        ...
        * 그 객체를 이용해 삽입/ 삭제/ 연산 할 때 차이가 남
        * */

        //리스트 계열에서  ArrayList 를 사용하겠다. 표기
