...
public class ArrayEx {
    ...
    *
    * - 생성된 배열의 요소는 index 를 이용해서 구분 (요소=값)
    * - 생성된 배열을 활용하기 위해서는 배열을 참조하는 "참조형 변수"를 이용
    *
    * [참조형 변수]
    * - 주소를 저장하는 변수
    * - 변수 사용 시 저장된 주소로 찾아가서, 그곳에 위치한 배열, 객체 참조
    * ** 기본 자료형 8개를 제외한 나머지 자료형은 모두 참조형 변수임
    *    기본 자료형 : boolean, short, int, long, byte, float, double, char
    * */
    public void method1(){
        //변수 선언
        int num;
        //Stack 영역에 int 자료형을 저장할 수 있는 변수(공간)을 생성하고
        //그 변수(공간)의 이름을 num으로 지정
        ...
        //배열 할당
        arr = new int[4];
        //HEAP 영역에 int 4개짜리 배열을 할당한 후 생성된 배열의 주소를 arr 변수에 대입하는 것
    ...
    /*
    * 배열 길이 확인 length를 이용해 개수 확인
    *
    * */

    public void method3(){
        int num[] = {1,100,1000};
        System.out.println(num.length); //num에 숫자가 몇개 들어있는지 확인 가능
    }
