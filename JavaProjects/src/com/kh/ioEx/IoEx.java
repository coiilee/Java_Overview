...
* 1. java.io 패키지(폴더)
*   전통적인 입출력 방식으로 Stream(스트림)을 사용해 데이터를 처리함
...
*
* 2. java.nio 패키지(폴더)
*  New IO의 약자로 , 더 효율적이고 비동기적인 데이터 처리 지원
*  Path     /   Files : 파일 및 디렉토리(폴더) 작업
*  ByteBuffer         : 바이트 데이터를 저장 및 읽기
...
* 작업을 요청한 후 , 결과가 반환될 때까지 기다리는 방식
*
* 요청한 작업이 완료될 때까지 다음 작업이 시작되지 않음
* 단순하고 이해하기 쉬움, 하지만 시간이 오래 걸리는 작업에서 효율이 떨어질 수 있음
* 호출한 쪽은 결과를 기다려야 하므로 작업이 순차적으로 실행됨
* 예 ) 은행에서 번호표를 뽑고 창구에서 기다리는 것(이전 고객이 끝나야 내 차례가 옴)
*
*
* 비동기 ( =Asynchronous)
* 같은 시점이 아닌 서로 독립적으로 실행되는 것을 의미.
* 비동기적으로 실행되는 대화 : 여러 사람들이 서로 다른 이야기를 하며 대화가 겹치는 상황
* 작업을 요청한 것과 별개로 다른 작업이 진행되는 방식.
* 작업이 완료되면 알림을 통해 결과를 전달받음 * 알림 : 콜백 이벤트
* 호출한 쪽은 작언이 끝날 때까지 기다리지 않아도 되므로 병렬 처리가 가능함.
* 효율적이지만, 구조가 복잡해질 수 있음
* 예 ) 음식적에서 음식을 주문한 뒤 , 음식을 기다리지 않고 대화를 하거나 다른 일을 하다가 음식이 나오면 가져오는 것
* */
public class IoEx {
